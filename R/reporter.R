...
body <- c(
  ...
  "if (length(qc@metadata)) {",
  "  md <- qc@metadata",
  "  # Convert list values to readable single-line strings",
  "  to_str <- function(x) {",
  "    if (is.null(x)) return(NA_character_)",
  "    if (is.atomic(x) && length(x) <= 6) return(paste(x, collapse = ', '))",
  "    if (inherits(x, 'Date') || inherits(x, 'POSIXt')) return(as.character(x))",
  "    paste(utils::capture.output(str(x, max.level = 1)), collapse = ' ')",
  "  }",
  "  df <- data.frame(",
  "    key = names(md),",
  "    value = vapply(md, to_str, character(1)),",
  "    stringsAsFactors = FALSE",
  "  )",
  "  DT::datatable(df,",
  "    options = list(scrollX = TRUE, pageLength = 15, autoWidth = TRUE, dom = 't'),",
  "    rownames = FALSE,",
